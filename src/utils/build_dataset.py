...
def get_session():
    ...
    if s is None:
        s = requests.Session()
        retries = Retry(total=5, connect=5, read=5, backoff_factor=0.3,
...
def build_coco_subset(root="data/coco-mini", split="train", pct="2%", workers=16):
    ...
    ds = load_dataset("yerevann/coco-karpathy", split=f"{split}[:{pct}]")
    out_images = os.path.join(root, f"{split}_imgs"); os.makedirs(out_images, exist_ok=True)
